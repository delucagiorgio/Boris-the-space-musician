...
if (typeof (MidiPlayer) == 'undefined') {
     ...
     */
    let MidiPlayer = function(midi, target, loop, maxLoop, end_callback) {
        this.midi = midi;
    ...
    MidiPlayer.prototype.play = function() {
        ...
        let _this = this; // must be 'let', otherwise _this is public, and causes problem.
        let file = this.midi;
        let loop = this.loop;
    ...
    //
    // A private function, defined by 'let'. 
    // Original definition in [2] is: function loadRemote(path, callback) {
    let loadRemote = function(path, callback) {
        ...
        fetch.onreadystatechange = function() {
            if(this.readyState == 4 && this.status == 200) {
                ...
                if (IE_HACK) { // for IE. 
                    let t = BinaryToArray(fetch.responseBody).toArray();
                    let ff = [];
                    let mx = t.length;
